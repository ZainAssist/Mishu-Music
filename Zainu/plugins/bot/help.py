...
from Zainu.utils import help_pannel
from Zainu.utils.database import get_lang
from Zainu.utils.decorators.language import LanguageStart, languageCB
from Zainu.utils.inline.help import help_back_markup, private_help_panel
...
async def helper_private(client: app, update: Union[types.Message, types.CallbackQuery]):
    ...
    if is_callback:
        ...
        await update.reply_video(
            video="https://graph.org/file/4e9c4641e71f29f34e51d-beea1d40c2961c10cc.jpg",
...
async def helper_private(client: app, update: Union[types.Message, types.CallbackQuery]):
    ...
    if is_callback:
        ...
        _ = get_string(language)
        keyboard = help_pannel(_)
        await update.reply_video(video="https://graph.org/file/4e9c4641e71f29f34e51d-beea1d40c2961c10cc.jpg", caption=_["help_1"].format(SUPPORT_CHAT), reply_markup=keyboard)
...
@app.on_callback_query(filters.regex("zainu") & ~BANNED_USERS)
@languageCB
async def first_pagexx(client, CallbackQuery, _):
    menu_next = help_pannel(_)
    try:
        await CallbackQuery.message.edit_text(Zainu_Text, reply_markup=menu_next)
        return
    except:
        return
